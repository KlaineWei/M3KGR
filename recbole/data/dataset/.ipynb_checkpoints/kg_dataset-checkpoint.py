...
class KnowledgeBasedDataset(Dataset):
    ...
    def _load_img_text(self):
        ...
        i_emb_df = pd.read_csv(img_entity_path)
        d_emb_df = pd.read_csv(des_entity_path)
        emb_df = pd.read_csv(entity_path)
        i_tokens = i_emb_df['entity'].values.tolist()
        d_tokens = d_emb_df['entity'].values.tolist()
        tokens = emb_df['entity_id:token'].values
        token2id_dict = dict(zip(tokens, self.token2id(field='entity_id', tokens=tokens)))
        image_embs = torch.load(img_path)
        text_embs = torch.load(text_path)
        ...
        setattr(self, f"image_embs", image_embs)
        setattr(self, f"text_embs", text_embs)
        setattr(self, f"img_ids", img_ids)
        setattr(self, f"text_ids", text_ids)