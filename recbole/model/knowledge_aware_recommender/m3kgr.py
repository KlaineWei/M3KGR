...
class M3KGR(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.cts_weight = config['cts_weight']
        ...
        # self.projection_head_multi_1.append(torch.nn.ReLU())
        ...
        # self.projection_head_multi_2.append(torch.nn.ReLU())
        ...
        # self.projection_head_uni_1.append(torch.nn.ReLU())
        ...
        self.projection_head_uni_2.append(torch.nn.Linear(self.embedding_size * 2, self.embedding_size * 2, bias=False))
        self.projection_head_uni_2.append(torch.nn.BatchNorm1d(self.embedding_size * 2, eps=1e-12, affine=affine))
        # self.projection_head_uni_2.append(torch.nn.ReLU())
        self.projection_head_uni_2.append(torch.nn.Linear(self.embedding_size * 2, self.embedding_size, bias=False))
    ...
    def _get_ego_embeddings(self):
        ...
        if self.multi_step_cur < self.multi_step:
            ...
            uni_embs = torch.cat((entity_embeddings, multi_embs), 1).to(self.device)
    ...
    def forward_k(self):
        ...
        for aggregator in self.aggregator_layers_k:
            ego_embeddings = aggregator(self.A_in_4, ego_embeddings)
            norm_embeddings = F.normalize(ego_embeddings, p=2, dim=1)
            embeddings_list.append(norm_embeddings)
        kgat_all_embeddings = torch.cat(embeddings_list, dim=1)
        user_all_embeddings, entity_all_embeddings = torch.split(kgat_all_embeddings, [self.n_users, self.n_entities])
    ...
    def calculate_loss(self, interaction):
        ...
        user_all_embeddings_q, entity_all_embeddings_q = self.forward_q()
        with torch.no_grad():
        ...
        cts_moco_loss = self.moco_loss(cts_embedding_q, cts_embedding_k, u=True)
        e_cts_moco_loss = self.moco_loss(e_cts_embedding_q, e_cts_embedding_k, u=False)
        ...
        loss = mf_loss + self.reg_weight * reg_loss + self.cts_weight * (
    ...
    def update_attentive_A(self):
        ...
        drop_edge_3 = self.rand_sample(indices.shape[1], size=int(indices.shape[1] * 0.1), replace=False)
        indices_3 = indices.view(-1, 2)[torch.tensor(drop_edge_3)].view(2, -1)
        kg_score_3 = kg_score[torch.tensor(drop_edge_3)]
        A_in_3 = torch.sparse.FloatTensor(indices_3, kg_score_3, self.matrix_size).cpu()
        A_in_3 = torch.sparse.softmax(A_in_3, dim=1).to(self.device)
        
        drop_edge_4 = self.rand_sample(indices.shape[1], size=int(indices.shape[1] * 0.1), replace=False)
        indices_4 = indices.view(-1, 2)[torch.tensor(drop_edge_4)].view(2, -1)
        kg_score_4 = kg_score[torch.tensor(drop_edge_4)]
        A_in_4 = torch.sparse.FloatTensor(indices_4, kg_score_4, self.matrix_size).cpu()
        A_in_4 = torch.sparse.softmax(A_in_4, dim=1).to(self.device)
